    ...
    <html>
      ...
      <body>
        ...
        <div style="margin-top:30px; font-size:12px; color:#999; text-align:center;">
          Generated by SVV APP on ${_escapeHtml(new Date().toLocaleString())}
        </div>
...
// Indian number words (Rupees + Paise, no "Only" appended)
function amountInWordsIN(num){
  const x = Number(num || 0);
  if (!isFinite(x)) return "";

  const rupees = Math.floor(x + 1e-9);
  ...
  function wordsInt(n){
    ...
    let y = n;
    const crore = Math.floor(y / 10000000); y %= 10000000;
    const lakh  = Math.floor(y / 100000);   y %= 100000;
    const thou  = Math.floor(y / 1000);     y %= 1000;
    const hund  = y;
  ...
  let out = wordsInt(rupees) || "Zero";

  if (paise > 0) {
    out += " and " + (wordsInt(paise) || "") + " Paise";
  }
...
// Print pdf and save pdf functions
      ...
      <br><br><br><br>
        <div class="hdr">
          ...
          <div class="hdrRight">
            ...
            <div class="meta"><b>Date:</b> ${escapeHtml(fmtDDMMYYYY(header.invoice_date || ""))}</div>
 ...
 const daysTxt =
  ...
  <div class="sig">
    ...
    <div>Authorized Signatory</div>
        ...
        <div class="footerBar">
        ${header.install_terms ? `<div style="margin-top:8px;text-align:left;"><b>Installation Terms:</b><br><div style="white-space:pre-line;">${escapeHtml(header.install_terms).replace(/\n/g,"<br>")}</div></div>` : ``}
        <br><br><div style="margin-top:8px;text-align:center;">
          Generated by SVV APP on ${escapeHtml(fmtDDMMYYYYTime(new Date()))}
        </div></div>
